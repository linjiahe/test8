...
public interface WaKuangJl2Mapper extends BaseMapper<WaKuangJl2> {
    ...
    @Select("select IFNULL(sum(balance),0) from wa_kuang_jl2 where source_type in (8,10) and  create_time > #{startTime} and create_time <=#{endTime}")
    int yesterdaySumKjBalance(@Param("startTime") String startTime,@Param("endTime") String endTime);
    ...
    @Update("update wa_kuang_jl2 set status = #{status},version=version+1 where id = #{id} and version = #{version};")
