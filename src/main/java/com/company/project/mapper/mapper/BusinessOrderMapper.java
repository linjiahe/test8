...
public interface BusinessOrderMapper extends BaseMapper<BusinessOrder> {
    ...
    @Update("update business_order set surplus_balance=surplus_balance + #{balance} where id=#{id}}")
    int addSurplusBalance(@Param("id") String id, @Param("balance") BigDecimal balance);
